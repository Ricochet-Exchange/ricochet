...
contract RicochetLaunchpad is Ownable, SuperAppBase {
    ...
    function initialize(
        ...
        address beneficiary,
        uint256 outputRate,
        uint128 feeRate) external {

        require(address(inputToken) != address(0) && address(_launchpad.inputToken) == address(0) , "inputToken");
        require(address(outputToken) != address(0) && address(_launchpad.outputToken) == address(0), "outputToken");
      ...
      returns (bytes memory newCtx)
  {
    ...
    (, , uint128 totalUnitsApproved, uint128 totalUnitsPending) = _launchpad.ida.getIndex(
    ...
    int96 appFlowRate = _launchpad.cfa.getNetFlow(_launchpad.inputToken, address(this));
    (, int96 requesterFlowRate, , ) = _launchpad.cfa.getFlow(_launchpad.inputToken, requester, address(this));
  ...
  function getIDAShares(uint32 index, address streamer) external view returns (bool exist,
    ...
    (exist, approved, units, pendingDistribution) = _launchpad.ida.getSubscription(
                                                                  _launchpad.outputToken,
                                                                  address(this),
                                                                  _launchpad.outputIndexId,
                                                                  streamer);
  ...
  function getStreamRate(address streamer) external view returns (int96 requesterFlowRate) {
    (, requesterFlowRate, , ) = _launchpad.cfa.getFlow(_launchpad.inputToken, streamer, address(this));
  }
