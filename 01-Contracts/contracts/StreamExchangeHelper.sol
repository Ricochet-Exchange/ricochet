...
library StreamExchangeHelper {
    ...
    function _closeStream(
        ...
        require(
            int256(self.inputToken.balanceOf(streamer)) <=
                self.streams[streamer].rate * 8 hours,
            "!closable"
        );
    ...
    function _emergencyCloseStream(
        ...
        bool isJailed = ISuperfluid(msg.sender).isAppJailed(
            ISuperApp(address(this))
        );
    ...
    function _distribute(
        ...
        uint256 initialBalanceInput = ISuperToken(self.inputToken).balanceOf(
            address(this)
        );
